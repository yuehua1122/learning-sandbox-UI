...
class UploadExamPage(Screen):
    ...
    def populate_table(self):
        ...
        # 設置表頭
        header_layout = GridLayout(cols=10, size_hint_y=None, height=50, spacing=5)
        header_layout.add_widget(Widget(size_hint_x=None, width=20))
        headers = ["名稱", "科目", "開放時間", "作答時間", "考試類型", "考試代碼", "上傳題目", "上傳評分標準", "提示功能"]
        
        for header in headers:
            header_label = Label(text=header, font_size=18, font_name="BiauKai", color=(0, 0, 0, 1), 
        ...
        try:
            ...
            if not exams:
                ...
                for exam in exams:
                    # 每筆考試資料顯示在表格中
                    exam_id = exam[0]
                    row_layout = GridLayout(cols=10, size_hint_y=None, height=80, spacing=5)  # 每一行的佈局
                    ...
                    for item in exam[1:]:
                        ...
                        label = Label(text=item_text, font_size=18, font_name="BiauKai", color=(0, 0, 0, 1),
                                      halign='center', valign='middle', text_size=(250, None))  # 設置 text_size 來控制換行
